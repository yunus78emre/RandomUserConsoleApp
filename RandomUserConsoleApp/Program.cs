...
namespace RandomUserToPostgres
{
    class Program
    {
        ...
        {
            var users = await FetchUsersAsync(400);
            Console.WriteLine($"Toplam çekilen kullanıcı sayısı: {users.Count}");
            int successCount = 0;
            int errorCount = 0;
            ...
            {   
                ...
                catch (Exception ex)
                {
                    ...
                    await InsertLogAsync(new Log
                    {
                        LogLevel = "ERROR",
                        Message = $"Kullanıcı eklenemedi: {user.Email}",
            ...
            Console.WriteLine($"Toplam kullanıcı: {users.Count}, Başarılı: {successCount}, Hatalı: {errorCount}");
        }
        ...
        static async Task InsertLogAsync(Log log)
        {
            ...
            var logCmd = new NpgsqlCommand
            {
                ...
                CommandText = @"INSERT INTO logs (log_level, message, exception, stack_trace, created_at)
                        VALUES (@LogLevel, @Message, @Exception, @StackTrace, @CreatedAt)"
            };

            logCmd.Parameters.AddWithValue("@LogLevel", (object?)log.LogLevel ?? DBNull.Value);
            logCmd.Parameters.AddWithValue("@Message", (object?)log.Message ?? DBNull.Value);
            logCmd.Parameters.AddWithValue("@Exception", (object?)log.Exception ?? DBNull.Value);
            logCmd.Parameters.AddWithValue("@StackTrace", (object?)log.StackTrace ?? DBNull.Value);
            logCmd.Parameters.AddWithValue("@CreatedAt", log.CreatedAt);
